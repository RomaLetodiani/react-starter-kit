...
import { Drawer, DrawerContent, DrawerTrigger } from '@/components/ui/common/drawer';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger } from '@/components/ui/common/dropdown';
...
const Progress = () => {
  ...
  return <div className="fixed top-0 left-0 h-1 bg-blue-500 transition-all duration-200 ease-in-out" style={{ width: `${progress}%` }}></div>;
};
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    { name: 'Dashboard', to: paths.platform.dashboard.getHref(), icon: Home },
    checkAccess({ allowedRoles: [ROLES.ADMIN] }) && {
      name: 'Users',
      to: paths.platform.profile.getHref(), // TODO: change to users
      icon: Users,
    },
      ...
      <aside className="fixed inset-y-0 left-0 z-10 hidden w-60 flex-col border-r bg-black sm:flex">
        <nav className="flex flex-col items-center gap-4 px-2 py-4">
          ...
          {navigation.map((item) => (
            ...
            >
              <item.icon className={cn('text-gray-400 group-hover:text-gray-300', 'mr-4 size-6 shrink-0')} aria-hidden="true" />
              {item.name}
            </NavLink>
          ))}
        </nav>
        ...
        <header className="bg-background sticky top-0 z-30 flex h-14 items-center justify-between gap-4 border-b px-4 sm:static sm:h-auto sm:justify-end sm:border-0 sm:bg-transparent sm:px-6">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => navigate(paths.platform.profile.getHref())} className={cn('block px-4 py-2 text-sm text-gray-700')}>
                Your Profile
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem className={cn('block w-full px-4 py-2 text-sm text-gray-700')} onClick={() => logout.mutate({})}>
                Sign Out
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
